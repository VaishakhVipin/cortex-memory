#!/usr/bin/env python3
"""
🧠 Simple Cortex Memory Auth Server
FastAPI server for authentication and API key management (no Redis required)
"""
...
APP_NAME = "Cortex Memory Auth Server"
APP_VERSION = "2.0.3"
APP_DESCRIPTION = "🧠 Authentication and API Key Management for Cortex Memory"

@asynccontextmanager
async def lifespan(app: FastAPI):
    """Application lifespan events."""
    # Startup
    print("🚀 Starting Cortex Memory Auth Server...")
    ...
    # Shutdown
    print("🛑 Shutting down Cortex Memory Auth Server...")
...
async def get_api_key_user(request: Request, db: Session = Depends(get_db)):
    ...
    if request.url.path in ["/docs", "/redoc", "/openapi.json", "/health", "/api/v1/auth/health"]:
        return None
...
async def root():
    ...
    return {
        "message": "Welcome to Cortex Memory Auth Server",
        "version": APP_VERSION,
        "description": APP_DESCRIPTION,
        "docs": "/docs",
        "health": "/health",
        "auth_endpoints": "/api/v1/auth"
    }
